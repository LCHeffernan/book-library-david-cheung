const { expect } = require("chai");
const request = require("supertest");
const { Book } = require("../src/models");
const app = require("../src/app");

describe("/books", () => {
  before(async () => await Book.sequelize.sync({ force: true }));

  beforeEach(async () => {
    await Book.destroy({ where: {} });
  });

  describe("with no records in the database", () => {
    describe("POST /books", () => {
      it("creates a new book in the database", async () => {
        const response = await request(app).post("/books").send({
          title: "title001",
          author: "author001",
          genre: "genre001",
          ISBN: "isbn001",
        });
        expect(response.status).to.equal(201);

        const newBookRecord = await Book.findByPk(response.body.id, {
          raw: true,
        });
        expect(response.body.title).to.equal("title001");
        expect(newBookRecord.title).to.equal("title001");
        expect(newBookRecord.author).to.equal("author001");
        expect(newBookRecord.genre).to.equal("genre001");
        expect(newBookRecord.ISBN).to.equal("isbn001");
      });

      it("title must be exist", async () => {
        const response = await request(app).post("/books").send({
          author: "author001",
          genre: "genre001",
          ISBN: "isbn001",
        });
        expect(response.status).to.equal(400);
        expect(response.body).to.equal("Title is required");
      });

      it("title must not be empty", async () => {
        const response = await request(app).post("/books").send({
          title: "",
          author: "author001",
          genre: "genre001",
          ISBN: "isbn001",
        });
        expect(response.status).to.equal(400);
        expect(response.body).to.equal("Title must not be empty");
      });

      it("author must be exist", async () => {
        const response = await request(app).post("/books").send({
          title: "title001",
          genre: "genre001",
          ISBN: "isbn001",
        });
        expect(response.status).to.equal(400);
        expect(response.body).to.equal("Author is required");
      });

      it("author must not be empty", async () => {
        const response = await request(app).post("/books").send({
          title: "title001",
          author: "",
          genre: "genre001",
          ISBN: "isbn001",
        });
        expect(response.status).to.equal(400);
        expect(response.body).to.equal("Author must not be empty");
      });
    });
  });

  describe("with records in the database", () => {
    let books;

    beforeEach(async () => {
      books = await Promise.all([
        Book.create({
          title: "title001",
          author: "author001",
          genre: "genre001",
          ISBN: "isbn001",
        }),
        Book.create({
          title: "title002",
          author: "author002",
          genre: "genre002",
          ISBN: "isbn002",
        }),
        Book.create({
          title: "title003",
          author: "author003",
          genre: "genre003",
          ISBN: "isbn003",
        }),
      ]);
    });

    describe("GET /books", () => {
      it("gets all books records", async () => {
        const response = await request(app).get("/books");
        expect(response.status).to.equal(200);
        expect(response.body.length).to.equal(3);

        response.body.forEach((book) => {
          const expected = books.find((a) => a.id === book.id);
          expect(book.title).to.equal(expected.title);
          expect(book.author).to.equal(expected.author);
          expect(book.genre).to.equal(expected.genre);
          expect(book.ISBN).to.equal(expected.ISBN);
        });
      });
    });

    describe("GET /books/:id", () => {
      it("gets books record by id", async () => {
        const book = books[0];
        const response = await request(app).get(`/books/${book.id}`);

        expect(response.status).to.equal(200);
        expect(response.body.title).to.equal(book.title);
        expect(response.body.author).to.equal(book.author);
        expect(response.body.genre).to.equal(book.genre);
        expect(response.body.ISBN).to.equal(book.ISBN);
      });

      it("returns a 404 if the book does not exist", async () => {
        const response = await request(app).get("/books/12345");

        expect(response.status).to.equal(404);
        expect(response.body.error).to.equal("The book could not be found.");
      });
    });

    describe("PATCH /books/:id", () => {
      it("updates books genre by id", async () => {
        const book = books[0];
        const response = await request(app)
          .patch(`/books/${book.id}`)
          .send({ genre: "genre999" });

        const updatedBookRecord = await Book.findByPk(book.id, {
          raw: true,
        });

        expect(response.status).to.equal(200);
        expect(updatedBookRecord.genre).to.equal("genre999");
      });

      it("returns a 404 if the book does not exist", async () => {
        const response = await request(app)
          .patch("/books/12345")
          .send({ genre: "genrexxx" });

        expect(response.status).to.equal(404);
        expect(response.body.error).to.equal("The book could not be found.");
      });

      it("title must not be empty", async () => {
        const book = books[0];
        const response = await request(app)
          .patch(`/books/${book.id}`)
          .send({ title: "" });
        expect(response.status).to.equal(400);
        expect(response.body).to.equal("Title must not be empty");
      });

      it("author must not be empty", async () => {
        const book = books[0];
        const response = await request(app)
          .patch(`/books/${book.id}`)
          .send({ author: "" });
        expect(response.status).to.equal(400);
        expect(response.body).to.equal("Author must not be empty");
      });
    });

    describe("DELETE /books/:id", () => {
      it("deletes book record by id", async () => {
        const book = books[0];
        const response = await request(app).delete(`/books/${book.id}`);
        const deletedBook = await Book.findByPk(book.id, { raw: true });

        expect(response.status).to.equal(204);
        expect(deletedBook).to.equal(null);
      });

      it("returns a 404 if the book does not exist", async () => {
        const response = await request(app).delete("/books/12345");
        expect(response.status).to.equal(404);
        expect(response.body.error).to.equal("The book could not be found.");
      });
    });
  });
});
